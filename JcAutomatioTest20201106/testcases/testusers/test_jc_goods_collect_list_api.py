...
@ddt
class GoodsCollectListTestCase(unittest.TestCase):
    '''收藏列表'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_user.xlsx'), 'goods_collect_list')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_goods_collect_list(self, case):
        url = apiURL + case.url
        method = case.method
