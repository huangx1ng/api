...
@ddt
class SignContractTestCase(unittest.TestCase):
    '''签订合同'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_user.xlsx'), 'sign_contract')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_sign_contract(self, case):
        url = apiURL + case.url
        method = case.method
