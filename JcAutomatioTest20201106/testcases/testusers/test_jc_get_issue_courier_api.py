...
@ddt
class GetIssueCourierTestCase(unittest.TestCase):
    '''查找单位出库物流'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_user.xlsx'), 'get_issue_courier')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_get_issue_courier(self, case):
        url = apiURL + case.url
        method = case.method
