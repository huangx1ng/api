...
class GoodCollectTestCase(unittest.TestCase):
    ...
    def test_case_goods_collect(self, case):
        ...
        if eval(case.data).get('goods_id') == '*goodid*':
            case.data = case.data.replace(eval(case.data).get('goods_id'), str(self.goodid))
        if eval(case.data).get('goods_id') == '*goodsid*':
            case.data = case.data.replace(eval(case.data).get('goods_id'), str(self.goodsid))
