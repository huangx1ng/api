...
@ddt
class DeliveryDetailTestCase(unittest.TestCase):
    '''用户出库明细查询'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_user.xlsx'), 'delivery_detail')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_delivery_detail(self, case):
        url = apiURL + case.url
        method = case.method
