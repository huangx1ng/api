...
@ddt
class GoodsHistoryTestCase(unittest.TestCase):
    '''获取用户浏览记录'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_user.xlsx'), 'goods_history')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_goods_history(self, case):
        url = apiURL + case.url
        method = case.method
