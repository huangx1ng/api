...
from common.img_base64 import img_64


@ddt
class CommutHmOrderReceipetTestCase(unittest.TestCase):
    '''接口名'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_user.xlsx'), 'commit_hm_order_receipt')
    cases = excel.read_data_obj()
    http = HttpRequest()
    @data(*cases)
    def test_case_commit_hm_order_receipt(self, case):
        url = apiURL + case.url
        method = case.method
        ...
        if '*img*' in eval(case.data).get('img'):
            case.data = case.data.replace(eval(case.data).get('img'), img_64)
