...
@ddt
class RelationLoginTestCase(unittest.TestCase):
    '''登录关联账号'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_user.xlsx'), 'relation_login')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_relation_login(self, case):
        url = apiURL + case.url
        method = case.method
