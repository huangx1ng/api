...
@ddt
class SaceUserDataTestCase(unittest.TestCase):
    '''上传证件资料'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_user.xlsx'), 'save_user_data')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_save_user_data(self, case):
        url = apiURL + case.url
        method = case.method
