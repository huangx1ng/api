...
@ddt
class UpdateUsereInfoTestCase(unittest.TestCase):
    '''更新用户信息'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_user.xlsx'), 'update_user_info')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_update_user_info(self, case):
        url = apiURL + case.url
        method = case.method
