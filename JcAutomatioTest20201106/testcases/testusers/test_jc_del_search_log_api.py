...
class DelSearchLogTestCase(unittest.TestCase):
    '''删除搜索记录'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_user.xlsx'), 'del_search_log')
    ...
    def test_case_del_search_log(self, case):
        ...
        res = self.http.requests(method=method, url=apiURL + '/users/search_history', data=data_modify_pwd)
        data_ = res.json().get('data')
        searchid = []
        for i in data_:
        ...
        if '*' in eval(case.data).get('id'):
            case.data = case.data.replace(eval(case.data).get('id'), str(searchid[0]))
