...
@ddt
class MwssageCountTestCase(unittest.TestCase):
    '''站内信统计'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_user.xlsx'), 'message_count')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_message_count(self, case):
        url = apiURL + case.url
        method = case.method
