...
@ddt
class HmIssueOrderTestCase(unittest.TestCase):
    '''含麻回执单列表'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_user.xlsx'), 'hm_issue_order')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_hm_issue_order(self, case):
        url = apiURL + case.url
        method = case.method
