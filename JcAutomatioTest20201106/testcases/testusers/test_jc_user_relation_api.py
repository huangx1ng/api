...
@ddt
class UserRelationTestCase(unittest.TestCase):
    '''关联账号'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_user.xlsx'), 'user_relation')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_user_relation(self, case):
        url = apiURL + case.url
        method = case.method
