...
@ddt
class IntegralOrderList(unittest.TestCase):
    '''积分订单'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'jc_api.xlsx'), 'integral_order_list')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_integral_order_list(self, case):
        url = apiURL + case.url
        method = case.method
