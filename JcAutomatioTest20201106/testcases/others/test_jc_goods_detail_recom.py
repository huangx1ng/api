...
@ddt
class GoodsDetailRecomCase(unittest.TestCase):
    '''商品推荐详情'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'jc_api.xlsx'), 'goods_detail_recom')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_goods_detail_recom(self, case):
        url = apiURL + case.url
        method = case.method
