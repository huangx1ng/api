...
@ddt
class ValidUserCouponCase(unittest.TestCase):
    '''是否领取优惠券'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'jc_api.xlsx'), 'valid_user_coupon')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_valid_user_coupon(self, case):
        url = apiURL + case.url
        method = case.method
