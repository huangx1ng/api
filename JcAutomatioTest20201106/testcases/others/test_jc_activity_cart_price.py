...
@ddt
class ActivityCartPrice(unittest.TestCase):
    '''是否有购物车删除'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'jc_api.xlsx'), 'activity_cart_price')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_activity_cart_price(self, case):
        url = apiURL + case.url
        method = case.method
