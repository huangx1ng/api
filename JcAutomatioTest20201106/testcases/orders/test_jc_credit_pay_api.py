...
@ddt
class CreditPayTestCase(unittest.TestCase):
    '''资信支付'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_order.xlsx'), 'credit_pay')
    cases = excel.read_data_obj()
    http = HttpRequest()
    com = Common()
    order_num = com.notpay_order_num()

    @data(*cases)
    def test_case_credit_pay(self, case):
        url = apiURL + case.url
        method = case.method
