...
@ddt
class BalancePayTestCase(unittest.TestCase):
    '''余额支付订单'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_order.xlsx'), 'balance_pay')
    ...
    @data(*cases)
    def test_case_balance_pay(self, case):
        url =apiURL + case.url
        method = case.method
        ...
        if '*' in eval(case.data).get('order_number'):
            case.data = case.data.replace(eval(case.data).get('order_number'), self.order_num)
