...
@ddt
class OrderPriceTestCase(unittest.TestCase):
    '''订单付款信息'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_order.xlsx'), 'order_price')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_order_price(self, case):
        url = apiURL + case.url
        method = case.method
        ...
        print('返回码{}, {}'.format(response, response.json()))
