...
@ddt
class UserRefounPriceTestCase(unittest.TestCase):
    '''订单申请售后计算金额'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_order.xlsx'), 'user_refund_price')
    cases = excel.read_data_obj()
    http = HttpRequest()
    com = Common()
    order_num = com.order_list_num(3)[0]
    order_goods_id = com.order_goods_id(order_num)[0]

    @data(*cases)
    def test_case_user_refund_price(self, case):
        url = apiURL + case.url
        method = case.method
        ...
        if '*' in eval(case.data).get('order_number'):
            case.data = case.data.replace(eval(case.data).get('order_number'), str(self.order_num))
        if '*' in eval(case.data).get('ids'):
            case.data = case.data.replace(eval(case.data).get('ids'), str(self.order_goods_id))
