...
@ddt
class OrderListTestCase(unittest.TestCase):
    '''查看订单列表'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_order.xlsx'), 'order_list')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_order_list(self, case):
        url = apiURL + case.url
        method = case.method
        ...
        print('返回码{}，返回结果{}：'.format(response, response.json()))
