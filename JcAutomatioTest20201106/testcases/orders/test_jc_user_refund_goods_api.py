...
@ddt
class UserRefundGoodsTestCase(unittest.TestCase):
    '''售后订单商品'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_order.xlsx'), 'user_refund_goods')
    cases = excel.read_data_obj()
    http = HttpRequest()
    com = Common()
    order_num = com.order_list_num(6)[0]

    @data(*cases)
    def test_case_user_refund_goods(self, case):
        url = apiURL + case.url
        method = case.method
