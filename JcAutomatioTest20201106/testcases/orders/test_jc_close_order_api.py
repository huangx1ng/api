...
@ddt
class CloseOrderTestCase(unittest.TestCase):
    '''关闭订单'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_order.xlsx'), 'close_order')
    cases = excel.read_data_obj()
    http = HttpRequest()
    com = Common()
    orderid = com.notpay_order_id()

    @data(*cases)
    def test_case_close_order(self, case):
        url = apiURL + case.url
        method = case.method
        ...
        if '*' in eval(case.data).get('order_id'):
            case.data = case.data.replace(eval(case.data).get('order_id'), str(self.orderid))
