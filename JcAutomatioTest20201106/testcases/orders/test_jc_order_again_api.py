...
@ddt
class OrderAgainTestCase(unittest.TestCase):
    '''再次订购'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_order.xlsx'), 'order_again')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_order_again(self, case):
        url = apiURL + case.url
        method = case.method
        ...
        print('返回码{}，数据：{}'.format(response, response.json()))
        res = response.json()
