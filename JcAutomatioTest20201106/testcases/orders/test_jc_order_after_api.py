...
@ddt
class OrderAfterTestCase(unittest.TestCase):
    '''订单售后'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_order.xlsx'), 'order_after')
    cases = excel.read_data_obj()
    http = HttpRequest()
    com = Common()
    order_id = com.order_list_id(6)[0]

    @data(*cases)
    def test_case_order_after(self, case):
        url = apiURL + case.url
        method = case.method
        ...
        if '*' in eval(case.data).get('order_id'):
            case.data = case.data.replace(eval(case.data).get('order_id'), str(self.order_id))
