...
@ddt
class OrderFeedbackTestCase(unittest.TestCase):
    '''订单反馈'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_order.xlsx'), 'order_feedback')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_order_feedback(self, case):
        url = apiURL + case.url
        method = case.method
        ...
        if '*orderid*' in eval(case.data).get('order_id'):
            case.data = case.data.replace(eval(case.data).get('order_id'), str(order_id))
