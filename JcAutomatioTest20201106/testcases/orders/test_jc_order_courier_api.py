...
@ddt
class OrderCourierTestCase(unittest.TestCase):
    '''获取物流信息'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_order.xlsx'), 'order_courier')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_order_courier(self, case):
        url = apiURL + case.url
        method = case.method
