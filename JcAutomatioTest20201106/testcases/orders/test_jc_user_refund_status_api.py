...
@ddt
class UserRefundStatusTestCase(unittest.TestCase):
    '''订单售后状态'''
    excel = ReadExcel(os.path.join(DATA_DIR, 'api_order.xlsx'), 'user_refund_status')
    cases = excel.read_data_obj()
    http = HttpRequest()

    @data(*cases)
    def test_case_user_refund_status(self, case):
        url = apiURL + case.url
        method = case.method
